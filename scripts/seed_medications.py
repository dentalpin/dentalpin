...
async def resolve_clinic_id(session, clinic_id_arg: str | None) -> UUID:
    # Use a raw connection to avoid ORM mapper initialization (which
    # would resolve relationships we don't import).
    ...
    if clinic_id_arg:
        ...
        result = await conn.execute(
            text("SELECT 1 FROM clinics WHERE id = :id"),
            {"id": clinic_id}
        )
    ...
    result = await conn.execute(text("SELECT id FROM clinics"))
    ...
    if not ids:
        ...
        print(
            f"ERROR: {len(ids)} clinics found — pass --clinic-id explicitly.",
            file=sys.stderr,
        )
    sys.exit(1)
