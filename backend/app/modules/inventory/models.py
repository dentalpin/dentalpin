...
DB level, and every quantity change goes through an atomic single-UPDATE
path (:meth:`InventoryService._apply_movement`) — never read-modify-write.
PR #153's earlier inventory died on exactly this race; see roadmap #220.
"""
...
# Why a movement happened. ``consumption`` rows are written by the
# auto-deduction handler (odontology performance), the rest by staff.
MOVEMENT_REASONS = ("initial", "restock", "consumption", "adjustment", "correction")
