...
viewer and the annotation canvas. Pixel data stays in the referenced media
`Document`; this module owns the study index row + the clinic-scoped
renderer.
...
- `POST   /patients/{id}/studies/index`  — index a media document as a study (201 new, 200 already indexed); `imaging_viewer.studies.write`
- `GET    /studies/{id}/render`          — PNG render (422 when undecodable); `imaging_viewer.studies.read`
- `DELETE /studies/{id}`                 — soft-archive (204); `imaging_viewer.studies.write`
- `POST   /rvg/scan`                     — scan the clinic watch folder now; `imaging_viewer.rvg.write`
...
RVG queue tables: `RvgLink` (unique clinic + DICOM PatientID →
...
- `imaging_viewer.studies.read` — list / view / PNG render.
- `imaging_viewer.studies.write` — index / archive.
- `imaging_viewer.rvg.read` — approval queue / links.
- `imaging_viewer.rvg.write` — scan trigger / approve / reject / unlink (dentist + admin).
...
- Consumes `media.photo_uploaded` → auto-indexes DICOM and sniffed-DICOM
  uploads inside a savepoint (best-effort, never raises into the
  publisher, never poisons the upload session).
...
- Migrations on the `imaging_viewer` Alembic branch (single `iv_0001`,
  squashed per review),
...
- `pydicom` + `numpy` are backend dependencies of this module (`pyproject.toml`);
  without them tags stay empty and renders answer 422.
