<!--
  ...
  KNOWN GAP: creating a staff payroll profile needs a user_id. There's
  no confirmed "list clinic staff/members" endpoint/composable available
  yet, so user_id is a plain text UUID input for now — not a proper
  picker. Swap this for a real staff-picker once that endpoint is
  confirmed (ask for ClinicMembership's list endpoint/composable).
-->
    ...
    <UModal v-model:open="showStaffModal">
      <template #content>
        <div class="p-4 space-y-3">
          ...
          <UInput v-if="!editingStaffId" v-model="staffForm.user_id" :placeholder="t('payroll.staff.userIdPlaceholder')" />
