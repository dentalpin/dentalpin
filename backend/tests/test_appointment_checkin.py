...
Covers: token mint (auth + 404), QR PNG render (+ origin validation),
...
async def test_checkin_happy_path(
    ...
    qr = await client.get(
        ...
        params={"origin": "https://clinic.example"},
        headers=auth_headers,
    )
    ...
    done = await client.post("/api/v1/agenda/public/check-in", json={"token": token})
    assert done.status_code == 200
    assert done.json()["data"]["status"] == "checked_in"

    # Double scan is idempotent, not an error.
    again = await client.post("/api/v1/agenda/public/check-in", json={"token": token})
...
async def test_checkin_rejects_bad_tokens(client) -> None:
    assert (
        await client.post("/api/v1/agenda/public/check-in", json={"token": "garbage"})
    ).status_code == 401
    assert (
        await client.post(
            "/api/v1/agenda/public/check-in",
            json={"token": create_access_token(uuid4())},
        )
...
async def test_checkin_unknown_appointment_is_404(client) -> None:
    ...
    r = await client.post("/api/v1/agenda/public/check-in", json={"token": token})
    assert r.status_code == 404
...
@pytest.mark.asyncio
async def test_qr_rejects_non_http_origin(
    client, auth_headers, test_clinic, db_session: AsyncSession
) -> None:
    world = await _world(db_session, test_clinic, await _admin_id(client, auth_headers))
    r = await client.get(
        f"/api/v1/agenda/appointments/{world['appointment_id']}/check-in-qr",
        params={"origin": "ftp://evil.example/x"},
        headers=auth_headers,
    )
    assert r.status_code == 422
...
@pytest.mark.asyncio
async def test_public_checkin_is_rate_limited(client) -> None:
    # documents the rate-limit wiring without burning the test budget:
    # 21 rapid invalid-token posts must eventually 429 (limit 20/minute).
    statuses = set()
    for _ in range(21):
        r = await client.post("/api/v1/agenda/public/check-in", json={"token": "x"})
