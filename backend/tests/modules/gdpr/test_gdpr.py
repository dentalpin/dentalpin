...
@pytest.mark.asyncio
async def test_consent_grant_withdraw_keeps_audit(
    db_session: AsyncSession, test_clinic: Clinic, test_patient: Patient
):
    ...
    # Withdraw — same row, not a new one (audit continuity).
    ...
    assert items[0].withdrawn_at is not None
...
async def test_erasure_retained_when_retention_holds(
    ...
    # retention_years>0 means still under legal hold → retain.
    await RetentionService.create(
        db_session,
        test_clinic.id,
